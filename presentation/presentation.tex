...
\begin{block}{QC is a broad field}

\begin{itemize}
\item Algorithm design (math/computer science)
\item Quantum mechanics (physics)
\item building/implementation (engineering)
\end{itemize}
...
\begin{itemize}
\item Algorithm design (math/data science)
\item Quantum mechanics (physics)
\item Building/implementation (engineering) 
\end{itemize}
% USE BLACKBOARD 
\item Takes advantage of entanglement, superposition and interference
% These effect can be used to create algorithms not allowed by classical computation
\item Utilizes qubits instead of bits
% explain qubits as direction on a sphere
\item Quantum gates acts on quantum states
% gates as unitary transformations, single and multi gates
% Analogue to classical gates, some examples
\end{itemize}
\end{block}
\end{frame}
...
\begin{equation}
F(\ket{\psi},\ket{\varphi}) = |\bra{\psi}\ket{\varphi}|,\;\; F(\ket{\psi},\ket{\varphi}) \iff \ket{\psi} = \ket{\varphi}
\end{equation}
...
\item numerically integrate $\ket{\psi_{0}$ (solving Schrödinger eq.) with perturbed parameters to obtain $\ket{\psi_{f}'$.
\item calculate $F(\ket{\psi_{f},\ket{\psi_{f}')$
